...
var minimumRationalCubicControlParameterValue = -(1 - math.Sqrt(DblEpsilon))
var maximumRationalCubicControlParameterValue = 2 / (DblEpsilon * DblEpsilon)
...
func rationalCubicInterpolation(x, xL, xR, yL, yR, dL, dR, r float64) float64 {
	...
	if !(r >= maximumRationalCubicControlParameterValue) {
		// the following line commented out due to shadowing variable
		// var t float64 = (x - xL) / h
		var omt = 1 - t
		var t2 = t * t
		var omt2 = omt * omt

		return (yR*t2*t + (r*yR-h*dR)*t2*omt + (r*yL+h*dL)*t*omt2 + yL*omt2*omt) / (1 + (r-3)*t*omt)
	}
	return yR*t + yL*(1-t)
}
...
func minimumRationalCubicControlParameter(dL, dR, s float64, preferShapePreservationOverSmoothness bool) float64 {
	var monotonic = dL*s >= 0 && dR*s >= 0
	var convex = dL <= s && s <= dR
	var concave = dL >= s && s >= dR
...
func convexRationalCubicControlParameterToFitSecondDerivativeAtLeftSide(xL, xR, yL, yR, dL, dR, secondDerivativeL float64, preferShapePreservationOverSmoothness bool) float64 {
	var r = rationalCubicControlParameterToFitSecondDerivativeAtLeftSide(xL, xR, yL, yR, dL, dR, secondDerivativeL)
	var rMin = minimumRationalCubicControlParameter(dL, dR, (yR-yL)/(xR-xL), preferShapePreservationOverSmoothness)
...
func convexRationalCubicControlParameterToFitSecondDerivativeAtRightSide(xL, xR, yL, yR, dL, dR, secondDerivativeR float64, preferShapePreservationOverSmoothness bool) float64 {
	var r = rationalCubicControlParameterToFitSecondDerivativeAtRightSide(xL, xR, yL, yR, dL, dR, secondDerivativeR)
	var rMin = minimumRationalCubicControlParameter(dL, dR, (yR-yL)/(xR-xL), preferShapePreservationOverSmoothness)
	return math.Max(r, rMin)
}

/**  original file = rationalcubic.cpp
//
